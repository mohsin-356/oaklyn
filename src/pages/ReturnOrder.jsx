...
import { listSales, addReturn, listReturns, getRestaurantInfo, getActiveBusinessDateId, getBusinessDateId, getPrinterConfig, getCurrentUser } from '../utils/storage.js'
...
export default function ReturnOrder(){
  ...
  useEffect(()=>{
    const load = async () => { try { setSales(await listSales()) } catch {}; try { setExistingReturns(await listReturns()) } catch {} }
    load()
  ...
  const filtered = useMemo(()=>{
    ...
    return sales
      .filter(s => recBizIdEq(s, date))
      .filter(s =>
        String(s.token||'').includes(qq) ||
        (s.customerName||'').toLowerCase().includes(qq) ||
        (s.customerNumber||'').toLowerCase().includes(qq)
      )
  },[q, sales, date])
  ...
  const doReturn = async (rec) => {
    ...
    // Ask for confirmation before recording a return
    const confirmed = await dialog.confirm({
      title: 'Confirm Refund',
      description: `Are you sure you want to process this refund? Token: ${String(rec.token || '-')}`,
      confirmText: 'Confirm Refund',
      cancelText: 'No'
    })
    if (!confirmed) return
    // Full order return; store as a return record and it will be subtracted from totals
  ...
  function reprint(rec, currentBizId){
    ...
    const receiptHtml = `<!DOCTYPE html><html><head><meta charset="utf-8" />
      ...
      <body style="font-weight:700">
        ...
        ${rec.cashier ? `<div class="row"><div>Cashier</div><div>${esc(rec.cashier)}</div></div>` : ''}
        <div class="sep"></div>
        ${itemsHtml}
        <div class="sep"></div>
        ...
        <div class="row"><div><b>Total Refunded</b></div><div><b>Rs.${Number(rec.total||0).toFixed(2)}</b></div></div>
        <div class="sep"></div>
  ...
  async function printReceiptIframe(){
    ...
    console.log('=== PRINT REFUND RECEIPT START ===')
    ...
    try {
      ...
      console.log('=== PRINT REFUND RECEIPT END ===')
    }
  }
    ...
    <section>
      ...
      <div className="row" style={{marginBottom:12, justifyContent:'space-between'}}>
              ...
              return (
                <>
                  <div className="input-wrap" style={{minWidth:220}}>
                    <input ref={ref} className="input" type="date" value={date} onChange={(e)=>setDate(e.target.value)} />
                    <button type="button" className="input-icon-btn calendar-toggle" onClick={()=>{ const el = ref.current; if (el && typeof el.showPicker === 'function') el.showPicker(); else el?.focus() }} aria-label="Open date picker" title="Open date picker">
        ...
        <div className="row" style={{gap:8}}>
          <input className="input" placeholder="Search by token, name, phone..." value={q} onChange={(e)=>setQ(e.target.value)} />
          <div className="muted" style={{whiteSpace:'nowrap'}}>Count: <b>{filtered.length}</b></div>
        </div>
      </div>
      <div className="card">
        <div className="bill-head" style={{gridTemplateColumns:'140px 80px 1fr 100px 240px'}}>
          <div>Date</div>
          <div>Token</div>
          <div>Customer</div>
          <div>Total</div>
        ...
        {filtered.length===0 && <div className="muted">No sales found.</div>}
        {filtered.map(rec => (
          <div key={String(rec.id || rec._id || Math.random())} className="bill-row" style={{gridTemplateColumns:'140px 80px 1fr 100px 240px'}}>
